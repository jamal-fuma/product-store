class FumaFormBuilder < ActionView::Helpers::FormBuilder
  include FumaFormHelper
  ...
  def radio_button_group(method, values, options = {})
    ...
    selections    
    field_name, label, options = field_settings(method, options)
    fuma_group("radio", field_name, label, selections, options)    
  end
  ...
  def check_box_group(method, values, options = {})
    ...
    field_name, label, options = field_settings(method, options)
    fuma_group("check-box", field_name, label, selections, options)    
  end
end
